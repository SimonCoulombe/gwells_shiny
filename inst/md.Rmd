...
worktype_order <- tibble(
  worktype = c("construction","alteration", "decommission"), worktype_order = c(1,2,3)
)
### this step is slow..  should try and make faster..
zzzz <- full_data %>% 
  select(well_tag_number, contains("start_date"), contains("end_date")) %>%
  gather(key=key, value=date, -well_tag_number) %>%
  separate(key, sep ="_", into  = "worktype", extra = "drop") %>% 
  left_join(worktype_order) %>% 
  filter(!is.na(date)) %>% 
  group_by(well_tag_number) %>% 
  filter(date == max(date)) %>% 
  arrange(desc(worktype_order)) %>%
  slice(1) %>%
  ungroup()

z <- z %>% left_join(zzzz %>% select(well_tag_number, worktype))
```
