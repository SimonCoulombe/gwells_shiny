...
prepare_data <- function(start_date, end_date,  start_well_tag_number, end_well_tag_number, connection){
  ...
  geocode <- dbGetQuery(connection, 
                        paste0("select * FROM  wells_geocoded WHERE well_tag_number IN (", paste0(my_wtn, collapse = ","), ")")
  )
  message("download qa")
  ...
  #   dplyr::group_by(well_tag_number) %>% 
  #   dplyr::summarise(
  #     
  #     has_air_rotary = max(stringr::str_detect(toupper(drilling_method_code), "AIR_ROTARY")),
  #     has_auger = max(stringr::str_detect(toupper(drilling_method_code), "AUGER")),
  #     has_cable_tool = max(stringr::str_detect(toupper(drilling_method_code), "CABLE_TOOL")),
  #     has_driving = max(stringr::str_detect(toupper(drilling_method_code), "DRIVING")),
  #     has_dugout = max(stringr::str_detect(toupper(drilling_method_code), "DUGOUT")),
  #     has_duo_rotary = max(stringr::str_detect(toupper(drilling_method_code), "DUO_ROTARY")),
  #     has_excavating = max(stringr::str_detect(toupper(drilling_method_code), "EXCAVATING")),
  #     has_jetting = max(stringr::str_detect(toupper(drilling_method_code), "JETTING")),
  #     has_mud_rotary = max(stringr::str_detect(toupper(drilling_method_code), "MUD_ROTARY")),
  #     has_other = max(stringr::str_detect(toupper(drilling_method_code), "OTHER")),
  #     has_unk = max(stringr::str_detect(toupper(drilling_method_code), "UNK"))
  #     
  #   )
  ...
  z <- full_data %>%
    dplyr::mutate(
      cross_referenced = tidyr::replace_na(stringr::str_detect(comments, "xref|x-ref|cross ref|cross-ref"), FALSE),
...
prepare_all_data <- function(connection){
  ...
  # 
  # drilling_method <- dbGetQuery(connection, 
  #                               "select well_tag_number,drilling_method_code  from drilling_method"
  # )
  # 
  # 
  # 
